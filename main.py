...
def mpi_exec_function(casename, input_file, cores, exec_dir):        
    ...
    cmd = ["mpirun", "-np", str(cores),os.path.join(exec_dir,'run'), 'mpi_input.cfg']
    p = subprocess.Popen(
...
if __name__ == "__main__":
    ...
    returnValues = list(futures.map(partial(mpi_exec_function,input_file=mpi_input_file, cores=4, exec_dir=mpi_exec_dir),cases))
